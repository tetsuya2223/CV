...
## 発揮したバリュー
本プロジェクトでは、私含めてフロントエンド2名バックエンド1名のチーム体制で開発を行いました。  
開発開始時には、バックエンドのAPIがまだ完成していない状況だったため、フロントエンドのみでできることから開発を進めていきました。
具体的にはFigmaを見て、共通コンポーネントとして切り出せるコンポーネントをatomsに実装していきました。
その結果、バックエンドのAPIが完成した際にスムーズに繋ぎ込みを行うことに成功し、納期通りに成果物を納品することが出来ました。
